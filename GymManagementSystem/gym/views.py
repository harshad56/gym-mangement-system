...
def discover_more(request):
    ...
    context = {}
    return render(request, 'basic_fitness.html', context)


def discover_more1(request):
    # Any data you want to pass to the template
    context = {}
    return render(request, 'new gym training.html', context)


def discover_more2(request):
    # Any data you want to pass to the template
    context = {}
    return render(request, 'basic muscle course.html', context)


def discover_more3(request):
    # Any data you want to pass to the template
    context = {}
    return render(request, 'advanced muscle course.html', context)


def discover_more4(request):
    # Any data you want to pass to the template
    context = {}
    return render(request, 'yoga training.html', context)


def discover_more5(request):
    # Any data you want to pass to the template
    context = {}
    return render(request, 'body building course.html', context)
...
def user_profile(request):
    if request.method == "POST":
        ...
        Signup.objects.filter(user=request.user).update(
        ...
        if request.method == "POST":
            ...
            try:
                Packagetype.objects.create(
                    category=categoryid, packagename=packagename)
                error = "no"
            except:
                error = "yes"
    except:
...
def load_subcategory(request):
    ...
    subcategory = Package.objects.filter(
        category=categoryid).order_by('PackageName')
